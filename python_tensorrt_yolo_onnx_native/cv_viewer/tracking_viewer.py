...
class TrackingViewer:
    def __init__(self, res, fps_, D_max, duration, labels):
        ...
        self.tracklets = [] 
        self.labels = [name.strip() for name in labels.split(',')] if labels else []
    ...
    def render_2D(self, left_display, img_scale, objects, render_mask, isTrackingON):
        ...
        for obj in objects:
            if renderObject(obj, isTrackingON):
                ...
                pts = obj.bounding_box_2d

                if len(pts) == 0:
                    continue

                
                xs = [int(pts[i][0]) for i in range(4)]
                ys = [int(pts[i][1]) for i in range(4)]
                x_min, x_max = min(xs), max(xs)
                y_min, y_max = min(ys), max(ys)

                if 0 <= obj.raw_label < len(self.labels):
                    name = self.labels[obj.raw_label]
                else:
                    name = f"RAW {obj.raw_label}"

                

                top_left_corner = cvt(pts[0], img_scale)
                top_right_corner = cvt(pts[1], img_scale)
                bottom_right_corner = cvt(pts[2], img_scale)
                bottom_left_corner = cvt(pts[3], img_scale)
                ...
                ucol_pos = ((bottom_left_corner[0]+bottom_right_corner[0])/2)
                if bottom_left_corner[1] == bottom_right_corner[1]:
                    vrow_pos = bottom_left_corner[1]
                else:
                    vrow_pos = ((bottom_left_corner[1]+bottom_right_corner[1])/2)
                ...
                detections.append({
                    ...
                    'bbox': {
                        'topl': str(top_left_corner),
                        'topr': str(top_right_corner),
                        'bottoml': str(bottom_left_corner),
                        'bottomr': str(bottom_right_corner)
                    },
                    'cone_base_centre': str(f"[{ucol_pos},{vrow_pos}]")             
                })
                if obj.mask.is_init():
                ...
                cv2.putText(left_display, str(name), (int(position_image[0]) - 40, int(position_image[1]) - 12), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255, 255), 1)
                # cv2.putText(left_display, str(RAW_LABEL_MAP[int(obj.raw_label)]), (int(position_image[0]) - 40, int(position_image[1]) - 12), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255, 255), 1)
                cv2.putText(left_display, "ID " + str(obj.id), (int(position_image[0]) - 40, int(position_image[1]) - 25), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255, 255), 1)
                ...
                if math.isfinite(obj.position[2]):
                    ...
                    cv2.putText(left_display, text, (int(position_image[0]) - 40, int(position_image[1] + 5)),
                                cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255, 255), 1)
                    
                # CONFIDENCE TEXT
                conf_text = f"{obj.confidence:.0f}%"
                cv2.putText(left_display, conf_text, (int(position_image[0]) - 40, int(position_image[1] + 20)),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255, 255), 1)

            try:
                response = requests.post("http://localhost:5000/update", json=detections)
                response.raise_for_status()
                # print("[POST] OK")
            except requests.exceptions.RequestException as e:
                print(f"Failed to send detections to server: {e}")
